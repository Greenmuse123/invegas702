...
// Define the specific return type for event image uploads
export interface EventImageResult {
  path: string;
  fullPath?: string;
}

export class EventImageUploader extends BaseUploader<EventImageResult> {
  constructor(client: SupabaseClient) {
    super(client, BUCKETS.EVENTS);
  }

  /**
   * Validates event image files
  ...
  /**
   * Generates a path for event images
  ...
  /**
   * Formats the upload result for event images
   * @param data The upload result data
   * @param error The upload error if any
   * @returns Formatted upload result
   */
  protected formatResult(data: any, error: any): UploadResult<EventImageResult> {
    if (error) {
  ...
  public async getPublicUrl(path: string): Promise<string | null> {
    try {
      ...
      console.log('Generated public URL for event image:', data.publicUrl);
